...
### 🔐 后台托管模式（可选自建服务端）
- **后台集中管理渠道**：用站长账号登录（和普通用户同一个登录入口，进去自动落到 `/admin` 后台）后添加 API 渠道（地址 + 密钥 + 模型），访客只能看到渠道名称和模型，密钥不进浏览器。
- **凭据注入中继**：前端请求打到同源 `/api/relay/<渠道id>/`，由服务端补上真实地址与 `Authorization` 后转发，支持大体积 multipart 上传与 SSE 流式透传。
- **多渠道静默故障转移**：一个渠道生图失败时服务端自动换下一个渠道重试，直到出图成功或全部失败。切换过程对用户完全透明——响应头还没发出去，前端只看到一次成功，不会收到 5xx、不会闪断、也不会重试。判定「该不该换渠道」有明确边界：密钥无效、欠费、上游 5xx、连不上才换；提示词被拒、限流、用户自己的取消属于请求本身的问题，换渠道也救不了，直接把上游原话透回去。任务详情页保留完整尝试记录供管理员排查。
- **积分与卡密充值**：管理员设好每张图的积分单价，用户生成前先冻结、**只有真正出图成功才扣分**，失败自动全额返还，不会出现「失败了还扣钱」。充值走卡密兑换：后台批量生成卡密（形如 `GIP-DR75-SESX-S7Z6`），导出 CSV 拿去卖，用户在充值弹窗里粘贴兑换。卡密有未使用/已使用/已作废三态，谁在什么时候用了哪张都能查到，重复兑换会明确告诉你上次是谁什么时候用的。余额不足时服务端返回结构化错误，前端直接弹出充值弹窗并算出「还差多少积分」。
- **微信扫码关注公众号登录**：扫码或验证码两种模式，登录后自动建号、不重复注册。**未认证的订阅号也能用**——验证码模式整条链路都不碰 `access_token`，因此不需要 IP 白名单，也不会因为没认证就报 `48001`。注意扫码（带参二维码）模式**只有微信认证服务号能用**，而个人主体注册不了服务号、类型也不可互转，所以个人号去做认证同样解锁不了它——个人认证的真实收益是「获取用户基本信息」，能存下真实昵称和头像。服务端处理明文/兼容/安全三种模式的消息加解密（AES-256-CBC）、`msg_signature` 校验、消息去重，取不到昵称时回落成「微信用户1234」。二维码、回调地址、Token/AESKey 该填什么后台都给了可直接复制的值和分步说明。
- **四种访问方式**：开放访问、共享口令、多用户账号、微信登录。登录按 IP 限流（10 分钟 10 次失败锁定 10 分钟）。
- **多用户数据隔离**：给每个人开一个账号，各自的生图记录、收藏与设置互相看不到——图片始终只存在访问者自己的浏览器里，服务端不保存任何图片。口令支持一键随机生成（形如 `k7mq-3xf9`，去掉了容易抄错的字符），创建后可直接复制整份登录信息发给对方。
- **Agent 也归后台管**：在后台选好接入方式与对话/出图渠道，前端直接可用，用户不需要填任何 API 设置；关掉则前端连 Agent 入口都不显示。指定的渠道被停用或改了 API 模式时会自动回落，不会留下点进去就报错的按钮。
- **后台概览仪表盘**：首屏一眼看到今日出图请求数、成功率、活跃用户数和故障渠道数，每个数字带环比；下面两张交叉表回答「今天谁在用」和「走了哪些渠道」，可切今日 / 近 7 天 / 近 14 天。多用户模式下才有按人统计——其他模式服务端不知道请求来自谁，会直说而不是留白。
- **用量与渠道健康度**：后台能看到每条渠道的调用次数、成功率、平均耗时和最近错误，连续出现渠道自身的故障才会被标成「疑似故障」并在渠道页顶部汇总。提示词被拒、限流、用户中途取消都不算渠道的锅——故障转移会让一个坏请求把每条渠道都试一遍，把这些算进去会误伤整条链路。探测通过或手动确认都能消除标记，超过 6 小时没再出错也会自动过期。只记渠道、成败、耗时和时间——提示词和图片一个字都不记。
- **深度自检查出没余额的渠道**：连通测试只列模型，欠费的账号照样显示「正常」——这是"后台看着没事、前端一直报错"的主要来源。「深度自检」给每条渠道真发一次最小尺寸出图请求，按上游的真实回复分成能出图 / 没余额 / 密钥无效 / 模型不可用 / 限流 / 连不上六类，没余额的可以一键停用（密钥保留，充值后重新启用即可）。OpenAI 的欠费返回 429，判定顺序上刻意先认错误码再看状态码，不会误判成限流。
- **一键测全部 + 拖拽排序**：并发探测所有渠道的连通性，结果直接标在卡片上，探测通过的会自动消除「疑似故障」标记；故障转移顺序可以直接拖动卡片调整。
- **邮箱验证码自助注册**：多用户模式下开放注册，注册必须填真实邮箱并验过 6 位验证码才建号——既是防机器人刷号的第一道闸，也让「忘记密码」真正可用。服务端自己实现 SMTP（隐式 SSL / STARTTLS / 明文三种加密，QQ、163、阿里云、Gmail 都有预置），**零运行时依赖**，不引 nodemailer。验证码 10 分钟有效、单次可用，同邮箱 60 秒冷却且每日限 8 封，同 IP 每小时限 20 封，连错 5 次即作废；注册码与重置码互不通用。注册成功直接进后台并自动到账「注册赠送」积分（额度可配、可关）。验证码只在内存里，不落盘、不下发、不进日志。后台有「测试连接」和「发送测试邮件」两级自检，失败会指明卡在连接 / 握手 / 认证 / 投递哪一步。
- **登录 / 找回密码**：用户名 + 口令登录，登录页是左右分栏的产品化门面（左侧讲能力与「注册即送 N 积分」，右侧表单），窄屏自动收成单列。忘记密码走邮箱验证码重置，重置成功后不自动登录——让用户拿新口令再登一次，顺带确认新口令没记错。
- **邀请码（可选加码，默认关闭）**：邮箱验证码已经能挡机器人，所以邀请码默认不开；需要更严的把控时再勾上，两道闸同时生效。开启后生成一个邀请码，别人凭码自己注册账号，支持限名额和设有效期，随时能作废重发。「作废邀请码」只让码失效，不会顺手把注册开关也关掉。
- **站点自己命名**：后台「站点标题」同时决定浏览器标签、前端顶栏、登录页品牌区、以及**验证码邮件的标题与正文落款**——默认「绘想」，改一处就全站生效。
- **后台也能在手机上用**：窄屏下导航折成菜单、表单单列、明细表横向滚动。
...
```bash
cp .env.example .env
vi .env                      # 至少填 GIP_ADMIN_USER / GIP_ADMIN_PASSWORD（这就是站长账号）
...
- 后台：浏览器打开 `http://服务器IP:8080`，用站长账号（`GIP_ADMIN_USER` / `GIP_ADMIN_PASSWORD`）登录，会自动进入 `/admin`；在这里添加 API 渠道（地址 + 密钥 + 模型）。
- 前端：`http://服务器IP:8080`，访客只能看到渠道名称和模型，看不到地址与密钥。
- 请求经同源 `/api/relay/<渠道id>/` 由服务端注入凭据后转发，密钥不进浏览器。
- 支持**多渠道静默故障转移**：一个渠道生图失败时服务端自动换下一个渠道重试，直到出图成功。用户只发一次请求、只收一次响应，看不到中间的失败。
- 三种访问方式可选：开放访问、共享口令、多用户账号。选多用户账号时，用户可凭**邮箱验证码自助注册**，也可以由后台给每个人开账号（口令可一键随机生成并复制），各自的数据互相看不到。微信扫码登录还没上线，后台里只有一个标记「开发中」的占位入口。
- 支持**积分与卡密充值**：设好每张图的积分单价，**出图成功才扣分、失败不扣分**。卡密在后台批量生成、导出 CSV 拿去卖，用户在前端充值弹窗粘贴兑换，余额不足时弹窗会自动打开并算出还差多少。
- Agent 模式在后台的「Agent 模式」页开关：选好对话渠道（需要 OpenAI 兼容 + Responses API）后前端直接可用，不开则前端不显示 Agent 入口。
- 后台首屏「概览」看今日出图量、成功率、谁在用、有没有渠道坏了；「用量与健康」页看更长跨度的渠道调用量与逐条明细。渠道页支持一键测全部、深度自检（查出哪条没余额）与拖拽排序。多用户模式下还能开一个邀请码让别人自助注册（默认关闭）。

服务端零运行时依赖（只用 Node 内置模块），配置以 JSON 文件持久化在 Docker 卷里。生成的图片始终只存在访问者自己的浏览器，服务端不保存图片，用量统计也只记渠道、成败、耗时和时间。
