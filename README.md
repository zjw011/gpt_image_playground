...
- **微信扫码关注公众号登录**：扫码或验证码两种模式，登录后自动建号、不重复注册。**未认证的订阅号也能用**——验证码模式整条链路都不碰 `access_token`，因此不需要 IP 白名单，也不会因为没认证就报 `48001`。服务端处理明文/兼容/安全三种模式的消息加解密（AES-256-CBC）、`msg_signature` 校验、消息去重，取不到昵称时回落成「微信用户1234」。二维码、回调地址、Token/AESKey 该填什么后台都给了可直接复制的值和分步说明。
