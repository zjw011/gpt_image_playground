...
- **多用户数据隔离**：给每个人开一个账号，各自的生图记录、收藏与设置互相看不到——图片始终只存在访问者自己的浏览器里，服务端不保存任何图片。
...
- 三种访问方式可选：开放访问、共享口令、多用户账号。选多用户账号时，后台给每个人开一个账号，各自的生图记录互相看不到。

服务端零运行时依赖（只用 Node 内置模块），配置以 JSON 文件持久化在 Docker 卷里。生成的图片始终只存在访问者自己的浏览器，服务端不保存图片。

> ⚠️ 服务本身只讲 HTTP。上公网前请挂一层 HTTPS 反代（Caddy / nginx / Cloudflare），并到后台「访问与安全」里决定谁能进——默认的开放访问意味着任何知道地址的人都能用你的密钥生图。详见 [后台托管模式文档](docs/self-hosted-backend.md)。
>
