...
//
// 为什么不用网页授权（snsapi_userinfo）而选带参二维码：
// 网页授权要求用户**在微信内打开网页**，而这里的主场景是"电脑上扫码，电脑上出图"。
// 带参二维码两种用户都覆盖，而且天然要求用户关注公众号——这正是运营上想要的。
//
// 关于昵称头像：`cgi-bin/user/info` 只有**已认证服务号**能用（订阅号会返回 48001
// api unauthorized）。所以它是可选增强：拿不到就退化成「微信用户 abcd」，绝不因为
// 拉不到昵称而让登录失败。
...
export function describeErrcode(errcode, errmsg = '') {
  ...
  const known = {
    ...
    48001: '该接口未授权。订阅号无法调用拉取用户资料接口，昵称头像将退化为占位值',
    50002: '账号受限，请检查公众号状态',
  }
...
/**
 * 拉取关注者昵称头像。
 * 只有已认证服务号有这个权限，订阅号会返回 48001。所以这里**吞掉异常返回 null**，
 * 让登录流程继续走降级路径，而不是因为拿不到头像就把人挡在门外。
 */
