...
//                      体验更好，但该接口只对微信认证服务号开放；个人主体注册不了服务号，
//                      所以个人号走不通这条路，认证也解锁不了（详见 wechat.mjs 顶部说明）。
//
 ...
 * 按 openid 找号，找不到就建一个。
 *
 * 昵称头像是可选增强：`cgi-bin/user/info` 需要账号已认证（认证订阅号或认证服务号），
 * 未认证会返回 48001。所以拉不到就用「微信用户 xxxx」占位，
 * 绝不允许"拿不到昵称"变成"登录失败"。
 */
