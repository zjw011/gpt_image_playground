...
import ManagedChannelsPanel from './settings/ManagedChannelsPanel'
...
export default function SettingsModal() {
  ...
  // 用户侧不提供「Agent 配置」页签，渠道与模型全部由管理员下发；API 页签降级为只读的「我的渠道」。
  const manageableTab = (tab: SettingsTab) => !backendManaged || tab !== 'agent'
  ...
  useEffect(() => {
    // 后端托管模式下渠道与模型由管理员在后台配置，用户侧没有可编辑的页签，
    // 外部（比如 Agent 校验失败）请求打开 'api'/'agent' 时统一落到习惯配置。
    if (!showSettings) return
    if (settingsTabRequest) {
      setActiveTab(manageableTab(settingsTabRequest) ? settingsTabRequest : 'general')
      return
    }
    setActiveTab((current) => (manageableTab(current) ? current : 'general'))
  }, [settingsTabRequest, showSettings, backendManaged])
      ...
      >
        ...
        <div className="flex flex-1 min-h-0 flex-col sm:flex-row">
          ...
          <div className="flex-1 flex flex-col min-w-0 min-h-0 bg-transparent relative overflow-hidden">
            ...
            {activeTab === 'agent' && !backendManaged && (
              <AgentSettingsTab
                draft={draft}
            ...
            {/* 托管模式下用户只能查看和切换管理员下发的渠道，不能增删改渠道与模型 */}
            {activeTab === 'api' && backendManaged && <ManagedChannelsPanel />}

            {activeTab === 'api' && !backendManaged && (
